                  ...
                  <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '20px', maxWidth: '800px', margin: '0 auto 40px auto' }}>
                                        <div style={{ background: 'linear-gradient(135deg, rgba(139, 92, 246, 0.2), rgba(124, 58, 237, 0.1))', border: '2px solid rgba(139, 92, 246, 0.3)', borderRadius: '20px', padding: '24px' }}>
                                          ...
                                          {sorted.slice(0, 3).map((a: Asset, i: number) => <div key={a.ticker} style={{ marginBottom: '12px' }}><div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: 'white', fontWeight: 'bold' }}>{i+1}. {a.ticker}</span>span><span style={{ color: '#a78bfa' }}>{getProgress(a).toFixed(1)}%</span>span></div></div>)}
                                        </div>
                                        <div style={{ background: 'linear-gradient(135deg, rgba(34, 197, 94, 0.2), rgba(16, 185, 129, 0.1))', border: '2px solid rgba(34, 197, 94, 0.3)', borderRadius: '20px', padding: '24px' }}>
                                          ...
                                          {[...assets].sort((a: Asset, b: Asset) => getChange(b) - getChange(a)).slice(0, 3).map((a: Asset, i: number) => <div key={a.ticker} style={{ marginBottom: '12px' }}><div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: 'white', fontWeight: 'bold' }}>{i+1}. {a.ticker}</span>span><span style={{ color: getChange(a) >= 0 ? '#4ade80' : '#f87171' }}>{getChange(a) >= 0 ? '+' : ''}{getChange(a).toFixed(1)}%</span>span></div></div>)}
                                        </div>
                            </div>
                            <div style={{ background: 'rgba(30, 41, 59, 0.6)', borderRadius: '20px', padding: '30px', marginBottom: '30px' }}>
                              ...
                              {sorted.map((a: Asset, i: number) => <div key={a.ticker} style={{ marginBottom: '20px' }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}><span style={{ color: 'white', fontWeight: 'bold' }}>{a.ticker} - {a.name}</span>span><span style={{ color: '#94a3b8' }}>${a.current.toLocaleString()} / ${a.target.toLocaleString()}</span>span></div><div style={{ background: 'rgba(51, 65, 85, 0.5)', borderRadius: '10px', height: '24px', overflow: 'hidden' }}><div style={{ background: `linear-gradient(90deg, ${a.color}, ${a.color}aa)`, height: '100%', width: `${getProgress(a)}%`, borderRadius: '10px', transition: 'width 0.5s' }}></div></div></div>)}
                            </div>
                            <div style={{ background: 'rgba(30, 41, 59, 0.6)', borderRadius: '20px', padding: '30px' }}>
                                        ...
                                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '20px' }}>
                                          {assets.map((a: Asset) => <div key={a.ticker} style={{ background: 'rgba(51, 65, 85, 0.5)', borderRadius: '12px', padding: '20px' }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}><span style={{ color: 'white', fontWeight: 'bold', fontSize: '18px' }}>{a.ticker}</span>span><span style={{ color: getChange(a) >= 0 ? '#4ade80' : '#f87171', fontSize: '14px' }}>{getChange(a) >= 0 ? '+' : ''}{getChange(a).toFixed(1)}%</span>span></div>{editing === a.ticker ? <div style={{ display: 'flex', gap: '8px' }}><input type="number" value={newPrice} onChange={(e) => setNewPrice(e.target.value)} style={{ flex: 1, padding: '8px', borderRadius: '6px', border: 'none', background: '#1e293b', color: 'white' }} /><button onClick={() => updatePrice(a.ticker)} style={{ padding: '8px 16px', borderRadius: '6px', border: 'none', background: '#8b5cf6', color: 'white', cursor: 'pointer' }}>OK</button>button></div> : <div onClick={() => { setEditing(a.ticker); setNewPrice(a.current.toString()) }} style={{ cursor: 'pointer', padding: '8px', background: '#1e293b', borderRadius: '6px', textAlign: 'center' }}><span style={{ color: 'white', fontSize: '20px', fontWeight: 'bold' }}>${a.current.toLocaleString()}</span>span></div>}</div>)}
                                        </div>
                            </div>
                  </div>
          </div>
